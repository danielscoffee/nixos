...
license: MIT
metadata:
  source: https://github.com/mattpocock/skills/tree/885e2ca4d842d139e9aef4e48d366c63cb1b8013/skills/engineering/wizard
---
...
- For setup: `.env.example`, other checked-in environment templates, `README`, `docker-compose*`, framework config, and `.github/workflows/*` (every `secrets.*` / `vars.*` reference is a value the wizard must produce). Never read secret-bearing `.env` files or values.
- For a migration or transition: the current state, the target state, and the irreversible actions between them.
- Before writing: resolve target ownership. Treat Nix/Home Manager-managed paths as immutable, update their declarative source, and use the project's existing secret manager for durable secrets.
