...
Perform a read-only security audit of `${1:-current repository}`. Use a fresh `security-auditor` and add a QA or supply-chain review angle only when scope warrants it. Cover application and supply-chain risk as relevant.
